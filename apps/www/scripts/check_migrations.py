...
Compares the migration files in apps/www/migrations/*.sql against what a given
...
Usage:  python3 apps/www/scripts/check_migrations.py [production|staging]
        (default: production)
...
def main():
    ...
    local = sorted(os.path.basename(p) for p in glob.glob(os.path.join(root, "apps/www/migrations/*.sql")))
    if not local:
        sys.exit("no migrations found in apps/www/migrations/ — wrong working dir?")
