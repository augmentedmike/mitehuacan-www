...
emits resources/map-data/denue.js — loaded lazily by the app and searched like every other
tier.

Input:  poi/denue_puebla.csv (from denue_21_csv.zip, INEGI; refreshed ~2x/yr)
Output: resources/map-data/denue.js  const DENUE = {places: [{n, c: [lon,lat], k}]}
"""
import csv
import importlib
import json
...
def main():
    src = ROOT / "resources" / "poi" / "denue_puebla.csv"
    ...
    osm = []
    pj = ROOT / "resources" / "map-data" / "places.js"
    if pj.exists():
    ...
    out = ROOT / "resources" / "map-data" / "denue.js"
    out.write_text(
