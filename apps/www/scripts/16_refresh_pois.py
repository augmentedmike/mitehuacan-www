...
Run weekly by .github/workflows/refresh-pois.yml (and by hand whenever):
  ...
  3. rebuilds resources/map-data/pois.js (via 15_build_pois) and resources/map-data/places.js.
...
def build_places(raw):
    ...
    (ROOT / "resources" / "map-data" / "places.js").write_text(
...
def main():
    ...
    (ROOT / "resources" / "poi" / "pois_osm.json").write_text(
    ...
    (ROOT / "resources" / "poi" / "places_osm.json").write_text(
        json.dumps(com, ensure_ascii=False), encoding="utf-8")
    n = build_places(com)
    print(f"places: {n} commercial destinations -> resources/map-data/places.js")
