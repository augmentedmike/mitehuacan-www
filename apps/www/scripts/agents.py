...
logged, timed, and visible:
    ...
    python3 apps/www/scripts/agents.py run <name>       # run one agent, fully logged
    python3 apps/www/scripts/agents.py run full         # the whole weekly chain
    python3 apps/www/scripts/agents.py dash             # dashboard at http://localhost:8790
...
AGENTS = {
    ...
    "osm":        {"cmd": [PY, "apps/www/scripts/16_refresh_pois.py"],   "desc": "OSM POIs + places (Overpass)"},
    "denue":      {"cmd": [PY, "apps/www/scripts/17_build_denue.py"],    "desc": "DENUE establishments (needs resources/poi/denue_puebla.csv)"},
    "calles":     {"cmd": [PY, "apps/www/scripts/20_build_streets.py"],  "desc": "streets + esquinas (Overpass)"},
    "google":     {"cmd": [PY, "apps/www/scripts/discover/run.py", "google"],
                   "desc": "Google Maps discovery — sweeps space×category into google.db"},
    "instagram":  {"cmd": [PY, "apps/www/scripts/discover/run.py", "instagram"],
                   "desc": "Instagram discovery — needs YOUR session (sign in below)"},
    "facebook":   {"cmd": [PY, "apps/www/scripts/discover/run.py", "facebook"],
                   "desc": "Facebook discovery — needs YOUR session (sign in below)"},
    "google-verify":    {"cmd": [PY, "apps/www/scripts/discover/run.py", "google", "--verify"],
                         "desc": "re-check google records, mark permanently-closed ones dead (prune)"},
    "instagram-verify": {"cmd": [PY, "apps/www/scripts/discover/run.py", "instagram", "--verify"],
                         "desc": "re-check instagram handles, mark gone ones dead (prune)"},
    "facebook-verify":  {"cmd": [PY, "apps/www/scripts/discover/run.py", "facebook", "--verify"],
                         "desc": "re-check facebook pages, mark gone ones dead (prune)"},
    "fb_events":  {"cmd": [PY, "apps/www/scripts/discover/run.py", "fb_events"],
                   "desc": "Facebook events — fiestas & events per town (needs YOUR facebook session)"},
    "fb_events-verify": {"cmd": [PY, "apps/www/scripts/discover/run.py", "fb_events", "--verify"],
                         "desc": "drop removed/cancelled events (prune)"},
    "gov_events": {"cmd": [PY, "apps/www/scripts/discover/run.py", "gov_events"],
                   "desc": "government/traditional events — fiestas patronales (no login, derived+flagged)"},
    "resolve":    {"cmd": [PY, "apps/www/scripts/discover/resolve.py"],
                   "desc": "backfill coords for no-coord businesses via Google Maps"},
    "publish":    {"cmd": [PY, "apps/www/scripts/23_publish_discovery.py"],
                   "desc": "publish coord-bearing discovery places to the search layer"},
    "publish-events": {"cmd": [PY, "apps/www/scripts/25_publish_events.py"],
                       "desc": "normalize + publish all *_events.db to the calendar feed (events.js)"},
    "unify":      {"cmd": [PY, "apps/www/scripts/discover/unify.py"],
